...
- Agent is built with **LangChain v1** `createAgent`, which runs on LangGraph LLM access only through `ChatGroq` from `@langchain/groq`, with `maxRetries` and `modelFallbackMiddleware`. Do not use the raw `openai` SDK.
